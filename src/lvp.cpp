...
void Lvp::updateMenus()
{
	...
	if (hasGLWidget)
	{
		...
		if (childImage->getViewType() == GLWidget::MPV)
		{ // se o tipo de visualização for multiplanar
		...
		else if (childImage->getViewType() == GLWidget::VIEW3D)
		{ // se o tipo de visualização for 3D
	...
	if (hasImageViewer3D)
	{															// se a imagem ativa for uma imagem 3D
	...
	{ // se não houver imagem 3D ativa, zera os valores
...
void Lvp::open(string _file, bool novo)
{
	...
	if (!fileName.isEmpty() && QFile::exists(fileName))
	{
		...
		if (ext == "cor" or ext == "rpc" or ext == "dtp" or ext == "dts" or ext == "dtg")
		{ // o arquivo será plotado
...
void Lvp::correlation3D(CCorrelacao3D::Tipos tipo)
{
	...
	if (ok)
	{
		...
		foreach (DbmImageViewer *mdiChild, imagesList)
		{
			...
			if (cor->Go(mdiChild->pm3D, tipo, indice, numPlanos))
			{ // Executa funcao autocorrelacao Go.
...
void Lvp::writeSettings()
{ // Grava a posição e tamanho atuais da aplicação
...
void Lvp::readSettings()
{ // Carrega a última posição e tamanho usadas para a aplicação.
